...
.then(data => {
    ...
    data.forEach(bidDetail => {
        ...
        bidValueElement.innerHTML = `<div class="font-comic text-md md:text-xl "> $ <span class="font-anta "> ${bidDetail.bidAmount} </span> </div>`;


        const bidMailElement = document.createElement('div');
        bidMailElement.innerHTML = `<div class="font-anta mb-4  text-md md:text-xl">  <span class="font-anta text-md "> ${bidDetail.email}</span>: </div>`;
