...
.then(data => {
    ...
    data.forEach(bidDetail => {
        ...
        bidValueElement.innerHTML = `<div class="font-anta text-md "> $ <span class="font-anta "> ${bidDetail.bidAmount} </span> </div>`;


        const bidMailElement = document.createElement('div');
        bidMailElement.innerHTML = `<div class="font-anta mb-4  text-md">  <span class="font-anta text-md "> ${bidDetail.email}</span>: </div>`;
